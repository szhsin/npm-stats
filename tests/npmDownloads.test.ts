...
test("defaults to the current month, using UTC at year boundaries", () => {
  expect(getMonthRangeDefaults(NOW)).toStrictEqual({
    startMonth: "2025-09", endMonth: "2026-09", maxMonth: "2026-09",
  });
  expect(getMonthRangeDefaults(new Date("2026-01-01T00:30:00Z"))).toStrictEqual({
    startMonth: "2025-01", endMonth: "2026-01", maxMonth: "2026-01",
  });
});
...
test("on the first of a month, ranges stop at available history without an empty chunk", () => {
  ...
  expect(() => createDownloadRanges("2026-01", "2026-01", now)).toThrow(/not available/);
});
...
test("invalid, reversed, pre-history and future ranges are rejected", () => {
  for (const [start, end] of [
    ["", "2024-01"], ["2024-13", "2024-13"], ["2024-1", "2024-02"],
    ["2024-02", "2024-01"], ["2014-12", "2024-01"], ["2026-10", "2026-10"],
  ]) {
...
test("requests run in parallel and sum correctly when responses arrive out of order", async () => {
  ...
  const fetchMock = vi.spyOn(globalThis, "fetch").mockImplementation((url) =>
    new Promise<Response>((resolve) => pending.push({ url: String(url), resolve })),
  );
  const resultPromise = fetchPackageDownloads({
    packageName: " @szhsin/react-menu ", startMonth: "2021-01", endMonth: "2024-01",
  }, NOW);

  // All requests must be started before any response is available.
  expect(fetchMock).toHaveBeenCalledTimes(3);
  expect(pending.every(({ url }) => url.endsWith("/%40szhsin%2Freact-menu"))).toBeTruthy();
  ...
  expect(await resultPromise).toStrictEqual({
    packageName: "@szhsin/react-menu", startMonth: "2021-01", endMonth: "2024-01",
    totalDownloads: 60, startDate: "2021-01-01", endDate: "2024-01-31",
  });
});

test("zero downloads is a successful result", async () => {
  vi.spyOn(globalThis, "fetch").mockImplementation(async (url) => pointResponse(url, 0));
...
test("invalid package names and ranges never reach the network", async () => {
  ...
  for (const packageName of ["", " ", "react,vue", "https://npmjs.com/react", "@scope", "a/b"]) {
    await expect(fetchPackageDownloads({ ...query, packageName }, NOW)).rejects.toThrow(/Enter a package name/);
  }
  await expect(fetchPackageDownloads({ ...query, startMonth: "2025-01" }, NOW)).rejects.toThrow(/Start month/);
  expect(fetchMock).not.toHaveBeenCalled();
});

test("HTTP errors provide actionable messages", async () => {
  for (const [status, message] of [
    [404, /No download data found/], [409, /HTTP 409.*wait and try again later/],
    [429, /Rate limited.*429.*wait and try again later/], [503, /HTTP 503/],
  ] as const) {
...
test("one failed chunk rejects the total and aborts outstanding requests", async () => {
  ...
  vi.spyOn(globalThis, "fetch").mockImplementation(async (url, options) => {
    ...
    return signals.length === 2 ? new Response(null, { status: 500 }) : pointResponse(url, 100);
  });
  await expect(fetchPackageDownloads({ ...query, startMonth: "2021-01" }, NOW)).rejects.toThrow(/HTTP 500/);
...
test("network failures and malformed payloads cannot become totals", async () => {
  ...
  vi.spyOn(globalThis, "fetch").mockImplementation(async () => { throw failure; });
  const result = fetchPackageDownloads(query, NOW);
  await expect(result).rejects.toThrow(/may be rate limited.*connection issue.*wait and try again later/);
  await expect(result).rejects.toHaveProperty("cause", failure);
  await expect(result).rejects.not.toThrow(/HTTP \d+/);
  vi.restoreAllMocks();

  vi.spyOn(globalThis, "fetch").mockImplementation(async () => new Response("not json"));
  await expect(fetchPackageDownloads(query, NOW)).rejects.toThrow(/invalid response/);
  vi.restoreAllMocks();

  for (const body of [
    null, { error: "package not found" },
    ...
    await expect(fetchPackageDownloads(query, NOW)).rejects.toThrow(/unexpected download data/);
    vi.restoreAllMocks();
  }
});
...
test("cancelled requests preserve the abort error instead of suggesting rate limiting", async () => {
  ...
  vi.spyOn(globalThis, "fetch").mockImplementation(async () => { throw abortError; });
  await expect(fetchPackageDownloads(query, NOW, controller.signal)).rejects.toBe(abortError);
});
...
test("the actual API end date is retained when current-month reporting lags", async () => {
  vi.spyOn(globalThis, "fetch").mockImplementation(async () => Response.json({
    package: "react", start: "2026-09-01", end: "2026-09-12", downloads: 10,
  }));
  const stats = await fetchPackageDownloads({ ...query, startMonth: "2026-09", endMonth: "2026-09" }, NOW);
...
test("result labels use the requested months and distinguish partial coverage", () => {
  ...
  expect(hasPartialMonth({ ...query, totalDownloads: 0, startDate: "2024-01-01", endDate: "2024-12-31" })).toBe(false);
  expect(hasPartialMonth({ ...query, startMonth: "2015-01", totalDownloads: 0, startDate: "2015-01-10", endDate: "2024-12-31" })).toBe(true);
});
