...
test('responses are reused for 24 hours without extending expiry on a cache hit', async () => {
  ...
  const fetchMock = vi
    ...
    .mockImplementation(async (url) => pointResponse(url, downloads));
...
test('current-month data uses a new request URL when another day becomes available', async () => {
  const fetchMock = vi
    .spyOn(globalThis, 'fetch')
    .mockImplementation(async (url) => pointResponse(url));
  ...
  expect(
    (await fetchPackageDownloads(currentQuery, beforeMidnight)).endDate,
  ).toBe('2026-09-15');
  currentTime = afterMidnight.getTime();
  expect(
    (await fetchPackageDownloads(currentQuery, afterMidnight)).endDate,
  ).toBe('2026-09-16');
  expect(fetchMock).toHaveBeenCalledTimes(2);
});
...
test('HTTP failures, network failures, invalid JSON and invalid download data are never cached', async () => {
  for (const respond of [
    ...
    const fetchMock = vi
      .spyOn(globalThis, 'fetch')
      .mockImplementation(async (url) =>
        fail ? respond() : pointResponse(url),
      );
...
test('invalid author search data is not cached', async () => {
  vi.spyOn(globalThis, 'fetch').mockImplementation(async () =>
    Response.json({ objects: [{}], total: 1 }),
  );
  await expect(
    fetchAuthorDownloads({ ...query, authorName: 'example' }, NOW),
  ).rejects.toThrow(/unexpected author package data/);
  expect(entries.size).toBe(0);
});
...
test('unavailable or full browser storage does not prevent successful queries', async () => {
  const fetchMock = vi
    .spyOn(globalThis, 'fetch')
    .mockImplementation(async (url) => pointResponse(url));
  const deniedRead = vi.spyOn(storage, 'getItem').mockImplementation(() => {
    throw new Error('Storage blocked');
  });
  const deniedWrite = vi.spyOn(storage, 'setItem').mockImplementation(() => {
...
test('cache pruning bounds storage and removes expired entries without touching other settings', async () => {
  ...
  for (let index = 0; index < 201; index++) {
    currentTime++;
    await fetchPackageDownloads(
      { ...query, packageName: `package-${index}` },
      NOW,
    );
  }
  expect(entries.size).toBe(201);
  expect(entries.get('theme')).toBe('dark');
  expect([...entries.keys()].some((key) => key.endsWith('/package-0'))).toBe(
    false,
  );
