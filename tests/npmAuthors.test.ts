...
const query = { authorName: "example", startMonth: "2024-01", endMonth: "2024-12" };

afterEach(() => vi.restoreAllMocks());

test("selects at most 10 popular author packages, fetching all packages and chunks in parallel", async () => {
  const names = Array.from({ length: 22 }, (_, index) => `@example/package-${index}`);
...
test("no author matches returns an empty result without download requests", async () => {
  const fetchMock = vi.spyOn(globalThis, "fetch").mockImplementation(async () => searchResponse([]));
  expect(await fetchAuthorDownloads(query, NOW)).toStrictEqual({
    ...query, totalCombinedDownloads: 0, packageCount: 0, totalPackageCount: 0, packages: [],
  });
...
test("duplicate search entries count only once and missing metadata is optional", async () => {
  ...
  vi.spyOn(globalThis, "fetch").mockImplementation(async (input) => {
    const url = String(input);
    if (url.startsWith("https://registry.npmjs.org/")) {
      return Response.json({ total: 2, objects: [{ package: { name: "react" } }, { package: { name: "react" } }] });
    }
...
test("zero totals remain successful and tied packages have a stable name order", async () => {
  ...
  expect(result.packages.map((pkg) => pkg.packageName)).toStrictEqual(["alpha", "zebra"]);
});
...
test("invalid authors and ranges never send requests", async () => {
  ...
  for (const authorName of ["", "@", "someone author:else", "example&size=250", "https://npmjs.com/~example"]) {
    await expect(fetchAuthorDownloads({ ...query, authorName }, NOW)).rejects.toThrow(/Enter an npm author username/);
  }
  await expect(fetchAuthorDownloads({ ...query, startMonth: "2025-01" }, NOW)).rejects.toThrow(/Start month/);
  expect(fetchMock).not.toHaveBeenCalled();
});

test("failed search requests and malformed search data are errors, not empty results", async () => {
  for (const [status, message] of [[429, /Rate limited.*429.*wait and try again later/], [503, /author packages.*HTTP 503/]] as const) {
  ...
  for (const body of [null, {}, { total: -1, objects: [] }, { total: 1, objects: [{ package: {} }] }]) {
    const fetchMock = vi.spyOn(globalThis, "fetch").mockImplementation(async () => Response.json(body));
    await expect(fetchAuthorDownloads(query, NOW)).rejects.toThrow(/unexpected author package data/);
    expect(fetchMock).toHaveBeenCalledTimes(1);
    vi.restoreAllMocks();
  }
});
...
test("one failed package rejects the author's total and aborts every outstanding chunk", async () => {
  ...
  vi.spyOn(globalThis, "fetch").mockImplementation(async (input, options) => {
    const url = String(input);
    if (url.startsWith("https://registry.npmjs.org/")) return searchResponse(["react", "missing", "vue"]);
    signals.push(options!.signal!);
    return url.endsWith("/missing") ? new Response(null, { status: 404 }) : pointResponse(url, 100);
  });
  await expect(fetchAuthorDownloads({ ...query, startMonth: "2023-01" }, NOW)).rejects.toThrow(/Could not load downloads for “missing”/);
...
test("author download failures retain the possible rate-limit explanation when the browser hides the status", async () => {
  vi.spyOn(globalThis, "fetch").mockImplementation(async (input) => {
    const url = String(input);
    if (url.startsWith("https://registry.npmjs.org/")) return searchResponse(["react"]);
    throw new TypeError("Failed to fetch");
  });
  const result = fetchAuthorDownloads(query, NOW);
  await expect(result).rejects.toThrow(/Could not load downloads for “react”.*may be rate limited.*wait and try again later/);
...
test("current-month packages reuse available-day boundaries and retain partial coverage", async () => {
  vi.spyOn(globalThis, "fetch").mockImplementation(async (url) =>
    String(url).startsWith("https://registry.npmjs.org/") ? searchResponse(["react"]) : pointResponse(url, 100),
  );
  const result = await fetchAuthorDownloads({ ...query, startMonth: "2026-09", endMonth: "2026-09" }, NOW);
